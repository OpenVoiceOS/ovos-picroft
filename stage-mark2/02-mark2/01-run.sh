...
if [[ -f "${ROOTFS_DIR}/boot/firmware/config.txt" ]]; then
    rm "${ROOTFS_DIR}/boot/firmware/config.txt";
fi
install -v -m 0644 files/config.txt "${ROOTFS_DIR}/boot/firmware/config.txt"
...
install -v -m 0644 files/xvf351-flash "${ROOTFS_DIR}/usr/libexec/xvf351-flash"

install -v -m 0644 files/xvf351.dtbo "${ROOTFS_DIR}/boot/firmware/xvf351.dtbo"
